...
每次调用结束后自动通过全局 :class:`CostGuard` 记账并检查预算：
超出预算时抛 :class:`BudgetExceededError` 中断流水线。
...
# 项目根目录注入 sys.path，保证 tests 包可导入
...
def chat(
    ...
    调用完成后自动记账（record）并检查预算（check），超预算即抛
    ...
    Args:
        ...
        temperature: 采样温度；None 时使用 API 默认值。低温度可提升评分等
            任务的输出一致性。
        node_name: 发起调用的节点名，用于成本按节点归集；缺省 ``unknown``。
    ...
    Raises:
        BudgetExceededError: 当累计成本超过 ``BUDGET_YUAN`` 预算时抛出。
    """
    ...
    cost_guard.record(node_name, usage, model or _model())
    cost_guard.check()
...
def chat_json(
    ...
    Args:
        ...
        node_name: 发起调用的节点名，透传给 chat() 用于成本归集；
            缺省 ``unknown``。
